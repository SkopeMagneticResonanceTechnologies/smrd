...
def create_temp_dir():
    global temp_dir
    temp_dir = tempfile.mkdtemp(prefix='smrd-python-', suffix='-test')
...
def test_file_returns_none_when_no_acquisitions_present():
    ...
    with smrd.File(filename) as file:
...
<ismrmrdHeader xmlns="http://www.ismrm.org/ISMRMRD" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.ismrm.org/ISMRMRD smrd.xsd">
...
def test_file_can_read_and_write_headers():
    ...
    filename = os.path.join(temp_dir, "file.h5")
    header = smrd.xsd.CreateFromDocument(example_header)
    ...
    with smrd.File(filename) as file:
        dataset = file['dataset']
        assert smrd.xsd.ToXML(header) == smrd.xsd.ToXML(dataset.header)
