...
def test_header():
    head = smrd.ImageHeader()
    assert ctypes.sizeof(head) == 198


def test_new_instance():
    img = smrd.Image()
    eq_(type(img.getHead()), smrd.ImageHeader)
    eq_(img.getHead().data_type, smrd.DATATYPE_CXFLOAT)
    ...
    head.data_type = smrd.DATATYPE_CXFLOAT   # must set data_type
    img = smrd.Image(head, attribute_string=attr)
...
def test_set_head():
    ...
    head = smrd.ImageHeader()
    head.data_type = smrd.DATATYPE_CXDOUBLE
    head.channels = nchan
    ...
    eq_(img.data_type, smrd.DATATYPE_CXDOUBLE)
...
def test_initialization_sets_nonzero_version():
    ...
    image = smrd.Image.from_array(common.create_random_array((128, 128), dtype=np.float32))
...
def test_initialization_with_array():
    ...
    image = smrd.Image.from_array(image_data)
...
def test_initialization_with_array_and_acquisition():
    ...
    image_data = common.create_random_array((256, 128), dtype=np.float32)
    image = smrd.Image.from_array(image_data, acquisition=acquisition)
...
def test_initialization_with_array_and_header_properties():
    ...
    image = smrd.Image.from_array(image_data, **properties)
...
def test_serialize_and_deserialize():
    ...
    with io.BytesIO(b'') as stream:
        ...
        read_image = smrd.Image.deserialize_from(stream.read)
...
def test_to_and_from_bytes():
    ...
    read_image = smrd.Image.from_bytes(image.to_bytes())
...
def test_serialization_with_header_fields():
    ...
    image_data = common.create_random_array((128, 128), dtype=np.float32)
    image = smrd.Image.from_array(image_data, **common.create_random_image_properties())
