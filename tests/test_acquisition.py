...
def test_encoding_counters():
    idx = smrd.EncodingCounters()
    eq_(ctypes.sizeof(idx), 34)


def test_header():
    head = smrd.AcquisitionHeader()
    eq_(ctypes.sizeof(head), 340)


def test_new_instance():
    acq = smrd.Acquisition()
    eq_(type(acq.getHead()), smrd.AcquisitionHeader)
...
def test_clearing_unset_flag_does_not_set_other_flags():
    acquisition = smrd.Acquisition()

    assert acquisition.flags == 0, \
        "Fresh acquisitions should not have any flags set."

    acquisition.clearFlag(smrd.ACQ_FIRST_IN_ENCODE_STEP1)
...
def test_initialization_from_array():
    ...
    acquisition = smrd.Acquisition.from_array(data)
...
def test_initialization_from_arrays():
    ...
    acquisition = smrd.Acquisition.from_array(data, trajectory)
...
def test_initialization_sets_nonzero_version():
    ...
    acquisition = smrd.Acquisition.from_array(common.create_random_data())
...
def test_initialization_with_header_fields():
    ...
    acquisition = smrd.Acquisition.from_array(data, **fields)
...
def test_initialization_with_illegal_header_value():
    smrd.Acquisition.from_array(common.create_random_data(), version='Bad version')
...
def test_serialize_and_deserialize():
    ...
    with io.BytesIO() as stream:
        ...
        deserialized_acquisition = smrd.Acquisition.deserialize_from(stream.read)
...
def test_to_and_from_bytes():
    ...
    deserialized_acquisition = smrd.Acquisition.from_bytes(acquisition.to_bytes())
...
def test_serialization_with_header_fields():
    ...
    acquisition = smrd.Acquisition.from_array(data, trajectory, **properties)
    deserialized_acquisition = smrd.Acquisition.from_bytes(acquisition.to_bytes())
